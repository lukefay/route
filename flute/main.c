...
#include <signal.h>
#include <time.h>
#define _TIMESPEC_DEFINED // Luke Fay trigger skip of redefinition in pthread.h
#include <stdio.h>

#ifdef _MSC_VER
#define _WINSOCKAPI_   /* Prevent inclusion of winsock.h in windows.h */
#include <pthread.h>	// Luke Fay
#include <windows.h>
#include <io.h>
#include <crtdbg.h>
#else
...
void usage(void) {
  ...
  printf("                    [-v:int] [-V:str] [-w:int] [-W] [-x:int] [-X:int] [-y:ul] [-Y:ul] [-z:int]\n");
  printf("                    [-Z]\n\n");
#else
  printf(" [-S] [-t:ull] [-T:int] [-U] [-v:int]\n");
  printf("                    [-V:str] [-w:int] [-W] [-x:int] [-X:int] [-y:ul] [-Y:ul] [-z:int] [-Z]\n\n");
#endif
...
#else
  printf(" [-s:str]\n");
  printf("                      [-t:ull] [-U] [-v:int] [-V:str] [-w:int] [-W] [-Y:ul]\n\n");
#endif
  ...
  printf("                    -A option\n");
  printf("   -Y               For Sender: The number of encoding symbols to send per packet.\n");
  ...
  printf("                    default: 0\n");
#endif
  printf("   -y               Segment Duration in microseconds\n\n");
...
// Luke Fay  chmod(CloseFile,S_IRWXU |S_IRWXG|S_IRWXO );
  fclose(CloseFilePtr);
  //END

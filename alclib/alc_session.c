...
int open_alc_session(alc_arguments_t *a) {
  ...
  //printf("number of ALC sessions: %i\n", nb_alc_session);
  //fflush(stdout);
...
int read_alc_session(alc_arguments_t* a, unsigned long long tsi) {
    ...
    /* Create receiving thread for new ALC Session TSI */
    ...
    s->handle_rx_thread =
        (HANDLE)_beginthreadex(NULL, 0, (void*)rx_thread, (void*)s, 0, &s->rx_thread_id);
    // Receiver.s_id == 0, so pull that one back up
    //WaitForSingleObject(get_alc_session(0)->handle_rx_thread, INFINITE);
...
int add_alc_channel(int s_id, unsigned long long tsi, const char *port, const char *addr, const char *intface, const char *intface_name) {
  ...
  printf("Added ALC channels: %lld\tnb_channel: %d\tmax#: %d\n", s->tsi, s->nb_channel, s->max_channel);
  fflush(stdout);
...
int update_wanted_object(int s_id, unsigned long long toi,
    unsigned long long transfer_len,
    unsigned short es_len, unsigned int max_sb_len, int fec_inst_id,
    short fec_enc_id, unsigned short max_nb_of_es,
    unsigned char content_enc_algo, unsigned char finite_field,
    ...
    alc_session_t* s;
    wanted_obj_t* next_wanted_obj;
    wanted_obj_t* wanted_obj;

    lock_session();

    s = alc_session_list[s_id];
    next_wanted_obj = s->wanted_obj_list;

    while (next_wanted_obj != NULL) {

        wanted_obj = next_wanted_obj;

        if (wanted_obj->toi == toi) {
            //printf("OBJ TOI found\n");
            //fflush(stdout);
        ...
        next_wanted_obj = wanted_obj->next;
    }

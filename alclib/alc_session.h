...
typedef struct alc_session {
  ...
  HANDLE handle_lct_thread;		/**< handle to thread which receives packets from the channel */
  unsigned int lct_thread_id;		/**< identifier for thread which receives packets from the channel */
#else
  ...
  pthread_t lct_thread_id[MAX_ALC_SESSIONS];		/**< identifier for thread which receives packets from the channel */
#endif
...
/**
* This function reads another LCT channel (TSI)
* 
* 
*/
...
/**
 * This function updates object wanted from the session.
 *
 * @param s_id session identifier
 * @param toi transport object identifier
 * @param transfer_len transport object length
...
int update_wanted_object(int s_id,
	unsigned long long toi,
	unsigned long long transfer_len,
	unsigned short es_len, unsigned int max_sb_len, int fec_inst_id,
	short fec_enc_id, unsigned short max_nb_of_enc_symb,

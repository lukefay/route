...
fdt_t* decode_fdt_payload(char *fdt_payload) {
	...
	printf("parsed FDT XML parameters with %d file(s)\n", fdt->nb_of_files);
	fflush(stdout);
	...
	while(tmp_file != NULL) {
		...
		for(;; fdt_file = fdt_file->next) {
			...
			if(tmp_file->toi == fdt_file->toi) {
				...
				// Skip over this to find next available opening
				continue;
			}
			else {
				// At end of list, add new file
...
int load_fdt(fdt_t* fdt_db, fdt_t* instance) {
	...
	while (tmp_file != NULL) {
		...
		for (;; tmp_file = tmp_file->next) {
			//printf("File list TOI %llu (%s) of status %d\n", tmp_file->toi, tmp_file->location, tmp_file->status);
			//fflush(stdout);

			fdt_file->fec_enc_id = -1;
			fdt_file->fec_inst_id = -1;
			fdt_db->nb_of_files++;
			...
			fdt_file->next = tmp_file->next;
			fdt_file->prev = tmp_file;
			//tmp_file->next = fdt_file;

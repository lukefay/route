...
// Luke Fay 	unsigned int sbnBackup;
	...
	while(sbn < bs->N /*Malek El Khatib 22.07.2014*/ || seekBack /*END*/ ) {
		...
		if(fdt_inst_buf != NULL) {/*Malek El Khatib*/printf("MAKE SURE THAT IT IS NOT COMING HERE\n");/*END*/
...
int fdtbasedsend(flute_sender_t *sender, int tx_mode, arguments_t *a) {
  ...
  */
  ...
  while(file != NULL) { /* loop for sending all files in fdt */
	  ...
	  //Note that below configuration assumes that objects in FDT are listed in order they are available
...
// Luke Fay		  struct timeval fdt_time, send_time;		//send_time is used later to check time needed to send file	  
		  struct timeval fdt_time;	  

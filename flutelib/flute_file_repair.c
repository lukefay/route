...
int flute_file_repair(flute_receiver_t *receiver, arguments_t *a, char *sdp_buf) {
  ...
  if(a->alc_a.cc_id == Null) {
	...
	retval = add_alc_channel(new_s_id, a->alc_a.tsi, ports[i], addrs[0], a->alc_a.intface, a->alc_a.intface_name);
	  }
	  else {
	retval = add_alc_channel(new_s_id, a->alc_a.tsi, ports[i], addrs[i], a->alc_a.intface, a->alc_a.intface_name);
	  }
	...
	retval = add_alc_channel(new_s_id, a->alc_a.tsi, ports[0], addrs[0], a->alc_a.intface, a->alc_a.intface_name);

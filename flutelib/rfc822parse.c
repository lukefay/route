...
/****************
 * This function may be used for the Content-Location header to figure out
 * the media Location (filename).  Note, that the returned strings are
 * guaranteed to be lowercase as required by MIME.
 *
 * Returns: a pointer to the media location.
 */
const char*
rfc822parse_query_media_location(rfc822parse_field_t ctx)
{
    TOKEN t = ctx;
    const char* location;

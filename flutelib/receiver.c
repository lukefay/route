...
int recvfile(int s_id, char *filepath, unsigned long long toi,
  ...
  int point;
  int i = 0;
// Luke Fay  variable not used in this routine   FILE *fabcd;
#ifdef USE_OPENSSL
...
int fdtbasedrecv(int rx_memory_mode, BOOL openfile, flute_receiver_t *receiver) {
  ...
  BOOL is_all_files_received = FALSE;
...
// Luke Fay  Variable not used in this routine  FILE *fabcd;
  ...
  while(1) {
    ...
    while(next_file != NULL) {
      ...
      file = next_file;
      i = 0;
	  is_all_files_received = all_files_received(receiver->file_uri_table);
	...
	if (rx_memory_mode == 1 || rx_memory_mode == 2) {
      ...
      while(next_file != NULL) {
		...
		if (receiver->verbosity > 0) {
			printf("Memory mode is HIGH(0), alc_recv2 for channel %i, TOI %llu of length %llu\n", receiver->s_id, file->toi, file->transfer_len);
			fflush(stdout);
		}
	...
	//printf("Rx Parse URI File location %s\n", file->location);
	//fflush(stdout);
...
int filemodesession(int rx_memory_mode, BOOL openfile, alc_session_t* s) {
	...
	while (get_session_state(s->s_id) == SActive) {
		...
		if (rx_memory_mode == 1 || rx_memory_mode == 2) {
			...
			if (s->verbosity > 0) {
				printf("Memory mode is HIGH(0), alc_recv2 for Session ID %i, File %s of length %llu\n", s->s_id, file->location, file->transfer_len);
				fflush(stdout);
			}
			...
			if (s->verbosity == 4) {
				printf("Get updated Session FDT now for TOI %lld, %d files in SESSION STATE: %d\n", toi, s->ls->fdt->nb_of_files, s->state);
				fflush(stdout);
			}
		...
		if (file->md5 != NULL) {
			...
			if (md5 == NULL) {
				...
				set_wanted_object(s->s_id, file->toi, file->transfer_len, file->es_len,
					file->max_sb_len,
					file->fec_inst_id, file->fec_enc_id,
					file->max_nb_of_es, content_enc_algo, file->finite_field,
					file->nb_of_es_per_group
		...
		if (s->verbosity == 4) {
			printf("Session Parse URI file location: %s\n", file->location);
			fflush(stdout);
		}
...
#else
			printf("Session File received: %s (TOI=%llu)\n", file->location, toi);
#endif
			fflush(stdout);
		}
...
void* channel_in_file_mode(void *c) {
	...
	while (s->ls->fdt == NULL) {
		...
		if (get_session_state(ch->s->s_id) == SExiting) {
			printf("SESSION IS EXITING\n");
			fflush(stdout);
			//return -2;
		}
		else if (get_session_state(ch->s->s_id) == STxStopped) {
			printf("SESSION STOPPED\n");
			fflush(stdout);
			//return -3;
		}
	...
	if (s->verbosity == 4) {
		printf("CHANNEL %lld has %d FILES\n", s->tsi, s->ls->fdt->nb_of_files);
		fflush(stdout);
	}

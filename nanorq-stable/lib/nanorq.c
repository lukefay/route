...
nanorq *nanorq_decoder_new(uint64_t common, uint32_t scheme) {
  ...
  //uint16_t T = (short)(common & 0xffff) + 1;
  uint16_t T = (short)(common & 0xffff);
  //printf("F: %lld\tT: %d\n", F, T);
  //fflush(stdout);
...
bool nanorq_repair_block(nanorq *rq, struct ioctx *io, uint8_t sbn) {
  ...
  //printf("generated interim symbols\n");
  //fflush(stdout);
  ...
  //printf("generated interim parameters\n");
  //fflush(stdout);
  ...
  //printf("determined interim overhead\n");
  //fflush(stdout);
  ...
  //printf("generated interim symbol matrix\n");
  //fflush(stdout);
  ...
  //printf("generated interim matrix invert\n");
  //fflush(stdout);
  ...
  //printf("decoded interim repair rows\n");
  //fflush(stdout);
  
  write_repair_rows(rq, sbn, dec->K, io, &M, repair_mask);

  //printf("wrote interim repair rows\n");
  //fflush(stdout);

  om_destroy(&M);

  //printf("removed interim matrix\n");
  //fflush(stdout);

  return (nanorq_num_missing(rq, sbn) == 0);
}

int nanorq_decode(int argc, char* argv[]) {
    char* outfile = argv[1];
...
#ifdef _MSC_VER
    uint8_t packet[1428];  //  MAX_SYMB_LENGTH_IPv4_FEC_ID_0_3_130 == 1428
#else

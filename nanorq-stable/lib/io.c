...
#ifdef _MSC_VER
      _chsize(_io->fd, _io->filesize + _io->filesize);
#else
...
#ifdef _MSC_VER
  long pos = _lseek(_io->fd, 0, SEEK_CUR);
  _lseek(_io->fd, 0, SEEK_END);
  ret = _lseek(_io->fd, 0, SEEK_CUR);
  _lseek(_io->fd, pos, SEEK_SET);
#else
...
struct ioctx *ioctx_mmap_file(const char *fn, int t) {
  ...
  if (t) {
    ...
    fd = _open(fn, O_RDWR | O_CREAT | O_TRUNC); // create decoder
  }
#else
  if (t) {
      fd = open(fn, O_RDONLY);

...
$initAudio = "*init.m*";
$initVideo = "*init.m*";
$segTemplateAudio = "*track2_*.m*";
$segTemplateVideo = "*track1_*.m*";

$Delay=0.5;	#How much would the AST of the patched MPD be lagging the current system time, i.e. how far in future is the AST (in seconds)?
$PatchedMPD="MultiRate_Dynamic_Patched.mpd";
#$FLUTEReceiver="./";
#$FLUTEReceiver="C:/work/ATSC_ROUTE-masterUpdate/Work/Route_Receiver/Release";
...
$index=($channel - 1)*2 + 1;
$sdp1="SDP" . (string)$index . ".sdp";			#SDP to be used by receiver
$index=$index + 1;
$sdp2="SDP" . (string)$index . ".sdp";			#SDP to be used by receiver
$index=$index + 3;
...
# Start first flute receiver
$cmd="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -p:$port -t:0 -Q -Y:$encodingSymbolsPerPacket -J:$Log > logout0.txt &";
#$cmd="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port -t:0 -Q -Y:$encodingSymbolsPerPacket -J:$Log > logout0.txt &";
#$cmd="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port -t:0 -Q -Y:$encodingSymbolsPerPacket &";
...
$date_array = explode(" ",$micro_date);
$date = date("Y-m-d H:i:s",$date_array[1]);
file_put_contents ( "timelog.txt" , "Started Globbing USBD:" . $date . $date_array[0] . " \r\n" , FILE_APPEND );
...
#Ad start - Read the content from the usbd.xml file
...
#$videoBase = $bundleDescription->userServiceDescription[0]->deliveryMethod->broadcastAppService->basePattern[0];
#$audioBase = $bundleDescription->userServiceDescription[0]->deliveryMethod->broadcastAppService->basePattern[1];
file_put_contents ( "timelog.txt" , "MPD: " . $fullMPDUri . " \r\n" , FILE_APPEND );
file_put_contents ( "timelog.txt" , "S-TSID: " . $sTSIDUri . " \r\n" , FILE_APPEND );
file_put_contents ( "timelog.txt" , "GSID: " . $bundleDescription->userServiceDescription[0]['globalServiceId'] . " \r\n" , FILE_APPEND );
file_put_contents ( "timelog.txt" , "BaseURL: " . $bundleDescription->userServiceDescription[0]->deliveryMethod->broadcastAppService->basePattern[0] . " \r\n" , FILE_APPEND );

# Clear DASH Segment directories
#array_map('unlink', glob("$DASHContent/$audioBase/*"));
#array_map('unlink', glob("$DASHContent/$videoBase/*"));
array_map('unlink', glob("$DASHContent/*"));

$micro_date = microtime();
$date_array = explode(" ",$micro_date);
$date = date("Y-m-d H:i:s",$date_array[1]);
file_put_contents ( "timelog.txt" , "Started Globbing S-TSID:" . $date . $date_array[0] . " \r\n" , FILE_APPEND );
...
file_put_contents ( "timelog.txt" , "Video TSI: " . $tsi_v . " \r\n" , FILE_APPEND );
file_put_contents ( "timelog.txt" , "Audio TSI: " . $tsi_a . " \r\n" , FILE_APPEND );

$cmdv="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port  -t:$tsi_v -Q -Y:$encodingSymbolsPerPacket -J:$Log2 > logout1.txt &";
#$cmdv="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port  -t:$tsi_v -Q -Y:$encodingSymbolsPerPacket &";
if (substr(php_uname(), 0, 7) == "Windows") {
  pclose(popen("start /B ". $cmdv, "r"));
} else {
  exec($cmdv . " > /dev/null &");
}

# Start second flute sender
$cmda="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port  -t:$tsi_a -Q -Y:$encodingSymbolsPerPacket -J:$Log3 > logout2.txt &";
#$cmda="$FLUTEReceiver/flute -A -B:$DASHContent -m:$destIP -s:$sourceIP -p:$port  -t:$tsi_a -Q -Y:$encodingSymbolsPerPacket &";
if (substr(php_uname(), 0, 7) == "Windows") {
  pclose(popen("start /B ". $cmda, "r"));
} else {
  exec($cmda . " > /dev/null &");
}
...
$date_array = explode(" ",$micro_date);
$date = date("Y-m-d H:i:s",$date_array[1]);
file_put_contents ( "timelog.txt" , "Started Globbing MPD:" . $date . $date_array[0] . " \r\n" , FILE_APPEND );
...
for ($periodIndex = 0; $periodIndex < count($periods); $periodIndex++) {
	...
	$videoPTO = (int)$videoSegmentTemplate->getAttribute("presentationTimeOffset");
	...
	$audioPTO = (int)$audioSegmentTemplate->getAttribute("presentationTimeOffset");

...
<html>
  <head>
	...
	<title>ROUTE/DASH Player</title>
	<link href="style.css" rel="stylesheet" type="text/css">
  </head>

  <body id="demopage">
	<h3>ROUTE/DASH Player</h3>
...
var audio_num_list_entries=1; // Change for different number of audio tracks.
...
function playingNow(e)
{
	...
	//$.post(
	//	"mediaDuration.php",
	//	{DynMPD: JSON.stringify('./DASH_Content1/ManifestUpdate_Dynamic.mpd')},
	//	function(e) {
	//		console.log("Ran mediaDuration.php, result: " + e);
	//		totalTime = JSON.parse(e);
	//		var playDelay = totalTime * 1000;
	//		console.log("***************** PlayDelay, " + playDelay);
	//		setTimeout(function () {start(1)}, playDelay);
	//	}
	//);
}
...
function start(channel) {
	...
	$.post(
		...
		{channel:channel},
		function(response) {
			console.log("Cleaned up previous FLUTE sessions" + response);
		}
	);
	...
	$.post(
		...
		function(response) {
			...
			video.play();
			// Try playing with Resident Media Player (RMP)
			//$.post(
			//	"PlayFFMPEG.php",
			//	{mpdURL:mpdURL}
			//);
			...
			monitoringTime = 0;
			monitorProcess = setInterval(function () {monitor404Errors(mpdURL)}, monitoringInterval);
			fragmentErrorRegistry = [];
			fragmentLoadErrorCount = 0;
			
			if(reTuneAtAd && result[1] > 0) {
				for(var index = 0, processStarted = 0 ; index < (result.length - 2) ; index++) {
					if(result[index+2] == 0) continue;
...
function monitor404Errors(mpdURL) {
	...
	if(fragmentErrorRegistry.length > maxWindowLength) fragmentErrorRegistry.shift();
	var ErrorTH = 1;
...
function checkthis(ele) {   
	if(!ele.checked ) {
		...
		$('#settingsDiv').hide();
		for(var i=1;i<=audio_num_list_entries;i++){
			if(!(document.getElementById('button'+i)== null)) document.getElementById('button'+i).remove();
		}
		$.ajax({
			type: 'POST',
			url: "ReceiverConfig/writeAdSelFile.php",
	...
	if(document.getElementById('settingsbtn').style.visibility !== 'hidden') start(udchannel);
}
